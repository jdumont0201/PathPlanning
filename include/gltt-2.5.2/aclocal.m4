...
dnl Seems these subfunctions or whatever are unavailable
...
ttf_incdirs="$ac_ttf_includes /usr/lib/freetype/include /usr/local/freetype/include /usr/include/freetype /usr/include/freetype1 /usr/include/freetype2 /usr/include /usr/local/include /usr/local/ttf $TTF_INC"
test -n "$TTFDIR" && ttf_incdirs="$TTFDIR/include $TTFDIR $ttf_incdirs"
AC_FIND_FILE(freetype/freetype.h, $ttf_incdirs, ttf_incdir)
...
# Don't give up just yet. Try freetype-config.
freetype_config_cflags="`freetype-config --cflags`"
freetype_config_libs="`freetype-config --libs`"

CXXFLAGS="$CXXFLAGS $freetype_config_cflags"
LIBS="$LIBS $freetype_config_libs"
...
if test "$freetype_config_works" = NO; then
  if test "$ac_ttf_includes" = NO || test "$ac_ttf_libraries" = NO; then
...
if test "$freetype_config_works" != NO; then
  TTF_INCLUDES="$TTF_INCLUDES $freetype_config_cflags"
  TTF_LDFLAGS="$TTF_LDFLAGS $freetype_config_ldflags"
fi
